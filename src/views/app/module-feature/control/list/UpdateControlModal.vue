...
const props = defineProps({
  showUpdateControlModalFlag: Boolean,
  currentControl: Object,
  statusList: Array,
  frameworkList: Array,
  domainList: Array,
  updateControlLoadingFlag: Boolean,
  modalId: {
    type: String,
    default: "update_control_company_modal",
  }
});
const {t: $t} = useI18n()
const emit = defineEmits(['resetUpdateControlData', 'onSubmitUpdateControl']);
const resetUpdateControlData = () => {
  emit('resetUpdateControlData');
};

function onSubmitUpdateControl() {
  emit('onSubmitUpdateControl');
}

const validationList = ref([ ])

//for handle validation error in update controlCompany
const updateControlSchemaValidation = ref(
    object(
...
function setValidationObjects(){
  ...
  updateControlSchemaValidation.value = object(
      validate(validationList.value)
  )
}

const frameworkRef = toRef(props.currentControl, 'framework');

watch(frameworkRef, (newValue) => {
  if (newValue) {
    props.currentControl.domain = null
...
onMounted(() => {
  statements.value = []
  for (let i = 0; i < props.currentControl.statement; i++) {
    statements.value.push(props.currentControl.statement[i]);
...
<template>
  <Modal
      @close="resetUpdateControlData"
      :title="$t('modal.update-control')"
      :id="modalId"
  ...
  >
    <div class=" max-h-[70vh] overflow-y-auto overflow-x-hidden">
      <Form :validation-schema="updateControlSchemaValidation" @submit="onSubmitUpdateControl" v-if="showUpdateControlModalFlag">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-2 grid-cols-1 gap-2" :class="{'hidden':tabIndex===1}">
          <VSelect class="col-span-1"
                   v-model="currentControl.framework"
                   name="framework"
                   deselect-label=""
                   :is-multiple="false"
                   :track-by="'id'"
          ...
          <VSelect class="col-span-1"
                   ...
                   :title="$t('form.domain')"
                   label="title"
                   :is-disabled="!currentControl.framework"
                   :placeholder="''"
          ...
          <TextInput class="flex-grow"
                     ...
                     name="number"
                     :has-default-error-message="true"
                     v-model="currentControl.number"/>
          <VSelect v-model="currentControl.status"
                   name="status"
                   deselect-label=""
                   :is-multiple="false"
                   :has-default-error-message="true"
          ...
          <TextareaInput class="flex-grow col-span-2"
                         ...
                         name="description"
                         :has-default-error-message="true"
                         v-model="currentControl.description"/>
          ...
          <TextareaInput class="flex-grow col-span-2"
                         ...
                         name="quick_win"
                         :has-default-error-message="true"
                         v-model="currentControl.quick_win"/>
        </div>
        ...
        <div class="grid md:grid-cols-1 lg:grid-cols-1 grid-cols-1" :class="{'hidden':tabIndex===0}">
          <div class="grid grid-cols-1 overflow-y-auto max-h-[500px] gap-2 p-1">
            <TextInput class="flex-grow relative"
                       v-for="(statement,index) in currentControl.statement" :key="index"
                       :label="$t('form.title')"
...
<!--                <div class="cursor-pointer " @click="updateStatement()" v-if="index=== currentControl.statement.length-1">-->
...
<!--                </div>-->
        ...
        <div class="py-[6px]" v-if="updateControlLoadingFlag">
