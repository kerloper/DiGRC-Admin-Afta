...
const props = defineProps({
  showFilterPageModalFlag: Boolean,
  filterDataTemp: Object,
  modalId:{
    type: String,
    default: "filter_page_modal",
  },
  sectionList: Array,
  filterPageLoadingFlag: Boolean
});

const emit = defineEmits(['resetFilterPageData', 'onSubmitFilterPage', 'removeFilterData']);

const resetFilterPageData = () => {
  emit('resetFilterPageData');
};

const onSubmitFilterPage = () => {
  emit('onSubmitFilterPage');
};
...
<template>
  <Modal
      @close="resetFilterPageData"
      :title="$t('modal.filter-page-list')"
  ...
  >
    <div>
      <Form @submit="onSubmitFilterPage" class=""
            v-if="showFilterPageModalFlag">
        ...
        <div class="py-[6px]" v-if="filterPageLoadingFlag">
