...
const props = defineProps({
  showFilterRoleModalFlag: Boolean,
  filterDataTemp: Object,
  modalId:{
    type: String,
    default: "filter_role_modal",
  },
  sectionList: Array,
  filterRoleLoadingFlag: Boolean
});

const emit = defineEmits(['resetFilterRoleData', 'onSubmitFilterRole', 'removeFilterData']);

const resetFilterRoleData = () => {
  emit('resetFilterRoleData');
};

const onSubmitFilterRole = () => {
  emit('onSubmitFilterRole');
};
...
<template>
  <Modal
      @close="resetFilterRoleData"
      :title="$t('modal.filter-role-list')"
  ...
  >
    <div>
      <Form @submit="onSubmitFilterRole" class=""
            v-if="showFilterRoleModalFlag">
        <div class="grid md:grid-cols-2 lg:grid-cols-2 grid-cols-1 gap-2">
          ...
          <TextInput class="flex-grow"
                     ...
                     name="resource"
                     :has-default-error-message="true"
                     v-model="filterDataTemp.resource"/>
          <TextInput class="flex-grow"
                     :label="$t('form.module')"
                     name="module"
                     :has-default-error-message="true"
        ...
        <div class="py-[6px]" v-if="filterRoleLoadingFlag">
