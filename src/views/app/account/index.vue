...
const refreshGlobalValue = inject('refreshGlobalValue');
...
const avatarUploaderModalId = "upload_avatar_modal"
const showAvatarUploaderModalFlag = ref(false)
const avatarUploaderLoadingFlag = ref(false)

function openAvatarUploader(){
  showAvatarUploaderModalFlag.value = true;
  setTimeout(function () {
    const modal = document.getElementById(avatarUploaderModalId);
    modal.showModal();
  }, 100)
}

function closeAvatarUploaderModal(){
  showAvatarUploaderModalFlag.value = false
}

function onSubmitAvatarUploader(){
...
<template>
  <div>
    <div v-if="showAvatarUploaderModalFlag">
      <AvatarUploaderModal
          :modalId="avatarUploaderModalId"
          :avatarUploaderLoadingFlag="avatarUploaderLoadingFlag"
          :showAvatarUploaderModalFlag="showAvatarUploaderModalFlag"
          @closeAvatarUploaderModal="closeAvatarUploaderModal"
          @onSubmitAvatarUploader="onSubmitAvatarUploader"
      />
    </div>
            ...
            <div class="flex items-center flex-1 px-3 justify-start">
              ...
              >
                ...
                <div
                    ...
                    class="absolute bottom-0 right-0 flex items-center justify-center p-2 mb-1 mr-1 rounded-full bg-primary text-white cursor-pointer"
                >
            ...
            <div class="md:flex lg:flex xl:flex">
              ...
              <div class="relative w-full mt-10 bottom-0 items-end justify-items-center">
                <div class="absolute bottom-0 lg:end-0 flex gap-2">
                  ...
                  <BaseButton class="btn-outline" :class="{'btn-primary':currentTap==='history'}" size="xs" @click="router.push({name: 'accountHistory'})">
                    <div class="flex">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                           stroke="currentColor" class="size-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
                  ...
                  <BaseButton class="btn-outline" :class="{'btn-primary':currentTap==='notification'}" size="xs" @click="router.push({name: 'accountNotification'})">
                    <div class="flex">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                           stroke="currentColor" class="size-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
